...
. Après avoir installé et configuré votre IDE, atteindre la dernière étape de ce premier tutoriel (avec twig) à savoir :

image:lucky-number-42.png[lucky number in action, 500]

== Migration application PHP classique vers Symfony
...
CAUTION: *SPECIFICATIONS* :   L'utilisateur cherche à trouver un nombre retenu par l'application de façon « aléatoire », sur une plage d'amplitude allant de zéro à, disons, 50.
